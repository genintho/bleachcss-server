...
(function(window) {
    ...
    Probe.prototype._addSelector = function(url, text, existsInStyleSheet) {
        ...
        text.split(",").forEach(function(item) {
            ...
            if (selector.length) {
                ...
                if (
                    url &&
                    self._allSelectors[selector].files.indexOf(url) === -1
                ) {
    ...
    Probe.prototype._fcnCheckFallback = function(selector) {
        console.log("test selector", selector);
        try {
    ...
    Probe.prototype._sendBuffer = function() {
        ...
        this._buffer = [];
        this._log("Try to send Buffer", cloneBuffer.length, cloneBuffer);
        ...
        httpRequest.open(
            "POST",
            self.options.url + "?t=" + new Date().getTime()
        );
        httpRequest.send(JSON.stringify(data));
        console.log(self.options.url, data);
    };
